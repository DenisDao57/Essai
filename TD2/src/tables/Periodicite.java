...
public class Periodicite {
	private static int TableSize = 2; 				// Nombre de colones de la Table
	...
	private static String Delete(HashMap<Integer,String> Datas){
		...
		PreparedStatement requete = laConnexion.prepareStatement(
				"DELETE from Periodicite WHERE "+ Table.get(1) + " = ?");
	...
	private static String Insert(HashMap<Integer,String> Datas) {
		...
		try {
			Connection laConnexion = Connexion.creeConnexion();
			PreparedStatement requete = laConnexion.prepareStatement(
					"INSERT INTO Periodicite (" +Table.get(2)+") Values(?)",Statement.RETURN_GENERATED_KEYS);
	...
	private static String Update(HashMap<Integer,String> Datas) {
        ...
        try {
             Connection laConnexion = Connexion.creeConnexion();
             PreparedStatement requete = laConnexion.prepareStatement(
                     "UPDATE Periodicite SET "+ Table.get(2)+ " =? WHERE "+ Table.get(1) +" =?");
             for (int i=1;i<=KeySize;i++) {
